...
def train_model(model, X_train, y_train, scats, location, name, config):
    ...
    model.save(save_location + '/' + location + '.h5')
    df = pd.DataFrame.from_dict(hist.history)
    df.to_csv(save_location + '/' + location + '_loss.csv',
              encoding='utf-8', index=False)


def train_ml_model(model, X_train, y_train, scats, location, name, config):
    ...
    with open(save_location + '/{location}' + '.pkl', 'wb') as f:
        pickle.dump(model, f)
    print(f"Training completed for {name} with MSE: {mse} and MAPE: {mape}")


def train_seas(models, X_train, y_train, scats, location, name, config):
    ...
    train_model(saes, X_train, y_train, scats, location, name, config)
...
def main(argv):
    ...
    if args.model == 'lstm':
        ...
        train_model(m, X_train, y_train, scats_num,
                    location, args.model, config)
    elif args.model == 'gru':
        X_train = np.reshape(X_train, (X_train.shape[0], X_train.shape[1], 1))
        ...
        train_seas(m, X_train, y_train, scats_num,
                   location, args.model, config)
    elif args.model == 'random_forest':
        m = RandomForestRegressor(n_estimators=100, random_state=42)
        train_ml_model(m, X_train, y_train, scats_num,
                       location, args.model, config)
